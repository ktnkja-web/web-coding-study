...
function movePlayer(pid, locationName) {
  ...
  return set(locRef, locationName)
    .then(() => {
      if (locationName !== "間取り図" || locationName !== "邂逅") {
...
function switchCharacter(id) {
  if (id === currentCharId) return; // 同じキャラなら何もしない
  ...
  if (currentItemRef && currentItemCallback) {
    off(currentItemRef, currentItemCallback); // ✅ 第2引数"value"は不要
  }
...
function showItems(id) {
  if (currentItemRef && currentItemCallback) {
    off(currentItemRef, currentItemCallback); // ✅ "value"削除
  }
